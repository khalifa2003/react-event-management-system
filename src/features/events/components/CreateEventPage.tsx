...
const CreateEvent: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.status && !['draft', 'published'].includes(formData.status)) return 'Invalid status';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    data.append('capacity[availableSeats]', formData.capacity.totalSeats.toString());
    data.append('capacity[soldSeats]', formData.capacity.totalSeats.toString());
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="h-64 w-full rounded border border-gray-300">
              <MapContainer
                center={[30.0444, 31.2357]} // Default: Cairo, Egypt
                zoom={10}
